...
from lib.complex_kanji_solver import ComplexKanjiSolver
from pages.campus.complex_kanji_page import ComplexKanjiPage
from services.campus.medal_service import MedalService


class ComplexKanjiService():
    """
    難読漢字を解く。
    ...
    def __init__(self, page, repo, setting):
        self.page = page
        self.complex_kanji_page = ComplexKanjiPage(page)
        self.solver = ComplexKanjiSolver()
        self.medal_service = MedalService(page=page,setting=setting)
        self._repo = repo
        self.setting = setting
        self._max_combo_attempts = self.setting.campus.complex_kanji["max_combo_attempts"]
    ...
    async def game_start(self):
        ...
        while True:
            try:
                if await self.complex_kanji_page.is_finished():
                    await self.medal_service.run()
                    break

                await self._run()

                await self.complex_kanji_page.transfer_check()

            except Exception as e:
                print(f"Complex Kanji Error: {e}")
                # タイムアウトするときは大概広告のせい
                await self.complex_kanji_page.close_ad()
    ...
    async def _solve_one_combo(self) -> None:
        ...
        # 1. DB検索（枝刈りあり）
        db_combo = self.solver.find_kanji_combo(available, self._repo)
    ...
    async def _retry(self) -> bool:
        ...
        # 正解 → next クリック
        await self.complex_kanji_page.click_next()
        await self._wait_for_selection_cleared()
        return True
    ...
    async def _try_combo(self, combo: tuple[str, str, str]) -> bool:
        ...
        await self.complex_kanji_page.click_check()
    ...
    async def _click_verbose_combo(self, combo: tuple[str, str]) -> None:
        ...
        for verbose in combo:
            locator = (
                self.page.locator('.dkanjiSelect:not(.isSelected):not(.isUsed)')
